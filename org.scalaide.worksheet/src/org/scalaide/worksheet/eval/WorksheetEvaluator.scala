...
import org.scalaide.worksheet.Configuration
...
class WorksheetEvaluator(scalaProject: ScalaProject) extends HasLogger {
  ...
  private val config: Configuration = Configuration(scalaProject)
  ...
  def eval(fullName: String, instrumented: Array[Char]): Either[EvaluationError, String] = {
    val instrumentedFile = writeInstrumented(fullName, instrumented)
    ...
    val binPath = config.binFolder.getAbsolutePath()
    val args = conf.buildArguments(List()).toList ++ List("-d", binPath)
    logger.debug("Compilation arguments: " + args)
    val (vdirOpt, reporter) = compileInstrumented(instrumentedFile.getAbsolutePath(), args)
   ...
   */
  private def writeInstrumented(iFullName: String, iContents: Array[Char]): File = {
    val sourceName = iFullName + ".scala"
    val sourceFile = config.touchSource(sourceName, iContents)
    sourceFile
  }
   ...
   */
  private def compileInstrumented(source: String, arguments: List[String]): (Option[AbstractFile], StoreReporter) = {
    logger.debug("compiling " + source)
    val command = new CompilerCommand(source :: arguments, println(_))
  ...
  def runInstrumented(mainClass: String): Either[EvaluationError, String] =  {
    ...
    try {
      ...
      val cp = (scalaProject.scalaClasspath.fullClasspath ++ scalaProject.outputFolderLocations.map(_.toFile) :+ config.binFolder).map(_.getAbsolutePath()).mkString("", File.pathSeparator, "")
